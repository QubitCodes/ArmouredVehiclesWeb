...
// Mock warranty claims data - set to empty array [] to see empty state
const mockWarrantyClaimsData: typeof mockWarrantyClaimsDataFull = [];

const mockWarrantyClaimsDataFull = [
  {
    id: "WC-001",
    ...
    price: 679,
    claimDate: "Monday, 4th Nov, 2024",
    status: "Claim Under Review",
    statusColor: "#D35400",
    issue: "Product defect - brake pads wearing unevenly",
    warrantyEndDate: "November 4th, 2025",
  },
  {
    id: "WC-002",
    orderId: "AMZ-12345678-123456",
    productName: "Duralast 45084DL High-Performance Disc Brake Rotor",
    productImage: "/order/wishlist3.svg",
    price: 475,
    claimDate: "Friday, 1st Nov, 2024",
...
export default function WarrantyClaimsPage() {
  const [claims] = useState(mockWarrantyClaimsData);
    ...
    <main className="flex-1">
      ...
      <div className="lg:hidden mb-4">
        ...
        <p className="font-inter text-xs text-[#666]">
          View your warranty claims history and submit a new claim for eligible items purchased through Armored Mart.
        </p>
      </div>
      ...
      {claims.length === 0 ? (
        ...
        <div className="flex flex-col items-center justify-center text-center py-16 lg:py-24">
          ...
          <Link href="/warranty-claims/new">
            <button className="bg-[#D35400] hover:bg-[#39482C] text-white clip-path-supplier-refund flex items-center justify-center w-full h-[45px] px-[30px] py-[15px] cursor-pointer">
              <span className="font-black text-[16px] font-orbitron uppercase">Start a New Claim</span>
            </button>
          </Link>
        </div>
      ) : (
        ...
        <div className="space-y-4">
          {claims.map((claim) => (
            <div 
              key={claim.id} 
              className="bg-[#F0EBE3] border border-[#CCCCCC] overflow-hidden"
            >
              {/* Claim Header */}
              <div className="px-4 py-3 lg:px-5 lg:py-4 border-b border-[#E8E3D9] flex items-center justify-between">
              ...
              <div className="p-4 lg:p-5">
                <div className="flex gap-4 lg:gap-5">
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="font-inter text-sm lg:text-base font-medium text-black mb-2 line-clamp-2">
                      {claim.productName}
                    </h3>
                    ...
                    <div className="flex items-center gap-1 mb-2">
                      ...
                      <span className="font-inter font-semibold text-sm text-black">
                        {claim.price.toLocaleString()}
                      </span>
                    </div>

                    {/* Claim Info */}
                    <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-[#666]">
                      <span>Claim filed: {claim.claimDate}</span>
                      <span>Order ID: {claim.orderId}</span>
                    </div>
                  </div>

                  {/* View Details Arrow */}
                  <button className="self-center text-[#666] hover:text-black transition-colors">
                    <ChevronRight size={24} />
                  </button>
                </div>

                {/* Claim Details */}
                <div className="mt-4 pt-4 border-t border-[#E8E3D9]">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <p className="font-inter text-xs text-[#666] mb-1">Issue Reported</p>
                      <p className="font-inter text-sm text-black">{claim.issue}</p>
                    </div>
                    <div>
                      <p className="font-inter text-xs text-[#666] mb-1">Warranty Valid Until</p>
                      <p className="font-inter text-sm text-black">{claim.warrantyEndDate}</p>
                    </div>
                  </div>
                </div>
              </div>
