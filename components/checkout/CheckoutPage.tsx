...
import { ArrowLeft, ChevronLeft, ChevronRight, Check, Package, Info, Phone, MapPin } from "lucide-react";
...
const mockAddress = {
  ...
  address: "2 23rd St, Al Khayl - Dubai Dubai Dubai, D - SW 0, Al Khayl - Dubai - Dubai, United Arab Emirates",
};
...
const paymentMethods = [
  { id: "card", label: "Debit/Credit Card", description: "Pay via your Visa and MasterCard etc.", icon: "visa", selected: true },
  { id: "tabby", label: "Tabby", description: "Split into 4. 0 payments", icon: "tabby" },
  { id: "tamara", label: "Tamara", description: "Pay in 4 intervals, interest free payments of AED 934.25", icon: "tamara" },
    ...
    <section className="bg-[#F0EBE3] min-h-screen">
      ...
      <div className="max-w-[1660px] mx-auto px-6 pt-6">
        ...
        >
          ...
          <span className="font-orbitron font-bold text-sm uppercase tracking-wider text-[#1A1A1A]">Back to Cart</span>
          <span className="text-[#6E6E6E] text-sm">(3 Items)</span>
        </button>
      </div>

      <div className="max-w-[1660px] mx-auto px-6 py-8 grid lg:grid-cols-[1fr_400px] gap-10 items-start">
        {/* LEFT COLUMN */}
        <div className="space-y-8">
          {/* SHIPPING ADDRESS */}
          <div>
            <h2 className="font-orbitron font-black text-2xl uppercase tracking-wide mb-4">
              Shipping Address
            </h2>
            
            {/* ADDRESS BOX */}
            <div className="bg-[#EBE3D6] p-5 mb-4">
              <p className="font-orbitron font-bold text-xs uppercase tracking-wider mb-3">Address</p>
              {/* Inner address card */}
              <div className="bg-[#F0EBE3] p-4 flex items-start justify-between">
                <div className="flex gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-[#6E6E6E] max-w-md leading-relaxed">
            ...
            <div className="bg-[#EBE3D6] p-5 mb-4">
              <div className="flex items-center justify-between mb-3">
                <p className="font-orbitron font-bold text-xs uppercase tracking-wider">Delivery Instructions</p>
                <button className="w-5 h-5 rounded-full bg-[#39482C] flex items-center justify-center hover:bg-[#2C3922] transition-colors">
                  <Info className="w-3 h-3 text-white" />
                </button>
              </div>
              <label className="inline-flex items-center gap-3 cursor-pointer bg-[#F0EBE3] px-4 py-2 border border-[#C2B280]">
                <div className="w-5 h-5 flex items-center justify-center bg-[#F0EBE3] border border-[#C2B280] shadow-sm"
            ...
            <div className="bg-[#EBE3D6] p-5">
              <p className="font-orbitron font-bold text-xs uppercase tracking-wider mb-4">Who will receive this order?</p>
              <div className="flex gap-3">
                {/* Self option */}
                <label 
                  className={`flex items-center gap-3 px-4 py-3 bg-[#F0EBE3] border cursor-pointer transition-all ${
                ...
                >
                  {/* Avatar */}
                  <div className="w-9 h-9 rounded-full bg-[#39482C] flex items-center justify-center flex-shrink-0">
                    <span className="text-white text-xs font-bold">JM</span>
                  </div>
                  <div className="flex-1">
                    <span className="text-sm font-semibold block">John Martin</span>
                    <p className="text-xs text-[#6E6E6E]">+91-77335-00000</p>
                  </div>
                  {/* Checkbox */}
                  <div 
                ...
                {/* Someone else option */}
                <label 
                  className="flex items-center gap-3 px-4 py-3 bg-transparent cursor-pointer transition-all"
              ...
              {mockShipments.map((shipment, index) => (
                <div key={shipment.id} className="bg-[#EBE3D6] p-5">
                  ...
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <span className="text-sm text-[#6E6E6E]">({shipment.items} Items)</span>
                  </div>
                  ...
                  {shipment.products.map((product) => (
                    <div key={product.id}>
                      <div className="flex gap-5">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-1 mt-1">
                            <svg width="14" height="12" viewBox="0 0 17 15" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                          ...
                          {"notReturnable" in product && product.notReturnable && (
                      ...
                      {/* Delivery Date - Bottom Right */}
                      <div className="flex justify-end mt-3">
                        <div className="text-right text-xs text-[#6E6E6E]">
                          <p>Get it by</p>
                          <p className="text-[#27AE60] font-semibold">{shipment.deliveryDate}</p>
                        </div>
                      </div>
                    </div>
                  ))}
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      {method.description && (
                        <p className="text-xs text-[#6E6E6E]">{method.description}</p>
                      )}
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    {method.id === "card" && (
                      <>
                        <div className="w-8 h-5 bg-[#DC143C] rounded flex items-center justify-center">
                    ...
                    {method.id === "tabby" && (
                      <div className="px-2 py-1 bg-[#3BFFC0] rounded text-[10px] font-bold text-black">tabby</div>
                    )}
                    {method.id === "tamara" && (
                      <div className="px-2 py-1 bg-[#FF6B00] rounded text-[10px] font-bold text-white">tamara</div>
                    )}
                    {method.id === "apple" && (
                      <div className="text-lg">🍎</div>
                    )}
                    {method.id === "paypal" && (
                      <div className="px-2 py-1 bg-[#003087] rounded text-[10px] font-bold text-white">PayPal</div>
                    )}
                  </div>
                  <input
                    type="radio"
        ...
        <div>
          ...
          <div className="sticky top-6 bg-[#EBE3D6]">
            <OrderSummary subtotal={subtotal} onCheckout={handleCheckout} buttonText="PLACE ORDER" />
          </div>
        </div>
      </div>
